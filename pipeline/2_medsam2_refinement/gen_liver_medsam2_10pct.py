"""
Liver MedSAM2 pseudo-labels for the 95 unlabeled LiTS cases (5% regime).
Label = supervised-seed liver (class 1) + MedSAM2 tumor (class 2). Logs Dice vs GT.
Adapted from KiTS gen_v2_pseudo_labels_medsam2_5pct.py. SAME method/hyperparams.
KEY DIFF: LiTS axial axis = 2 (spacing [0.7,0.7,5]); transpose axis2<->0 around the pipeline.
"""
...
SAM2_CONFIG = "configs/sam2.1/sam2.1_hiera_t512.yaml"
SAM2_CKPT = "/root/autodl-tmp/MedSAM2/checkpoints/MedSAM2_CTLesion.pt"
D507 = Path("/root/autodl-tmp/nnUNet_data/nnUNet_raw/Dataset507_Liver")
V3_DIR = Path("/root/autodl-tmp/predictions/liver_sup10_on_unlabeled90")
SPLIT = "/root/autodl-tmp/lits_ssl_split.json"
OUT_LABELS = Path("/root/autodl-tmp/lits_build/liver_medsam2_pseudo_labels_10pct")
OUT_JSON = "/root/autodl-tmp/lits_build/liver_medsam2_pseudo_label_quality_10pct.json"
CT_MIN, CT_MAX = -200.0, 300.0
TUMOR = 2
MIN_ABS, FRAC, BUF = 200, 0.25, 5
AX = 2  # LiTS axial axis
...
def main():
    ...
    cases = json.load(open(SPLIT))["unlabeled_10pct"]
    print(f"Generating liver MedSAM2 pseudo-labels for {len(cases)} cases", flush=True)
    ...
    for i, cid in enumerate(cases):
        ...
        v3 = np.moveaxis(v3_nii.get_fdata().astype(np.uint8), AX, 0)
        img = np.moveaxis(nib.load(str(D507 / "imagesTr" / f"{cid}_0000.nii.gz")).get_fdata().astype(np.float32), AX, 0)
        gt = np.moveaxis(nib.load(str(D507 / "labelsTr" / f"{cid}.nii.gz")).get_fdata().astype(np.uint8), AX, 0)
        ...
        combined[v3_k] = 1
        combined[sam2] = 2
        combined_orig = np.moveaxis(combined, 0, AX)
        out = nib.Nifti1Image(combined_orig, v3_nii.affine, v3_nii.header)
        ...
        rows.append(r)
        flag = "OK" if r["delta"] > 0.02 else ("~" if r["delta"] > -0.02 else "!!")
    ...
    print(f"  Mean V3 (seed) pseudo Dice: {v3m:.4f}", flush=True)
    print(f"  Mean v2 (MedSAM2) pseudo Dice: {v2m:.4f}", flush=True)
    print(f"  disasters (< -0.15): {len(disasters)} -> {disasters}", flush=True)
    print("=" * 60, flush=True)
