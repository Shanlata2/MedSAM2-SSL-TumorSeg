...
# ABLATION: pre-refinement supervised-seed predictions instead of MedSAM2-refined labels
PRE="/root/autodl-tmp/predictions/liver_sup5_on_unlabeled95"
OUT="/root/autodl-tmp/nnUNet_data/nnUNet_raw/Dataset509_liver_ours5gate"
TAU=0.80
gt=S["labeled_5pct"]; val=S["val"]
kept=sorted([c for c in S["unlabeled_5pct"] if CONF.get(c,0)>=TAU])
print(f"GT={len(gt)} gated={len(kept)} val={len(val)} total={len(gt)+len(kept)+len(val)}")
assert len(gt)==5 and len(val)==31
...
for c in kept:
    assert os.path.exists(f"{PRE}/{c}.nii.gz"), f"missing pre-refinement pred: {c}"
print("sources OK")
...
for c in kept:   shutil.copy(f"{PRE}/{c}.nii.gz", f"{OUT}/labelsTr/{c}.nii.gz")
...
print(f"Built D509: {len(glob.glob(OUT+'/imagesTr/*.nii.gz'))} imgs / {len(glob.glob(OUT+'/labelsTr/*.nii.gz'))} labels | train {len(gt)+len(kept)} val {len(val)}")